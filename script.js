...
function lockAssignment(reason) {
  if (Date.now() < ignoreSecurityUntil) return;
  if (isLocked) return;
  isLocked = true;
  recordSecurityEvent("נעילת מטלה", reason);

  const overlay = document.getElementById("lockOverlay");
  const reasonBox = document.getElementById("lockReason");
...
function unlockAssignment() {
  ...
  if (enteredCode === UNLOCK_CODE) {
    isLocked = false;
    ignoreSecurityUntil = Date.now() + 8000;
    recordSecurityEvent("שחרור נעילה על ידי מורה");
    ...
    input.value = "";
    showSecurityWarning("הנעילה שוחררה. המשיכו במטלה. מומלץ ללחוץ שוב על התחלת המטלה במסך מלא.");
  ...
  if (error) {
    ...
    error.textContent = "קוד שגוי. הקוד צריך להיות 248195";
  }
  recordSecurityEvent("ניסיון שחרור בקוד שגוי");
  return false;
}
...
async function handleSubmit(event) {
  ...
  if (isLocked) {
    showSecurityWarning("המטלה נעולה. יש לפנות למורה לשחרור.");
    return;
  }
